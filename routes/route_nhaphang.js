...
router.post("/nhaphang", async (req, res) => {
    ...
    if (inputData2 && inputData1) {
        ...
        request.input('InputData2', sql.NVarChar, inputData2);
        const result = await request.query(`EXEC FindInboundOrdersByTimePeriod null, @InputData2`);
        res.render('nhaphang', { dulieu: result.recordset });
    }
    else if (inputData1 && !inputData2) {
        request.input('InputData1', sql.NVarChar, inputData1);
        const result = await request.query(`EXEC FindInboundOrdersByTimePeriod @InputData1, null`);
        res.render('nhaphang', { dulieu: result.recordset });
    }
    else {
        const result = await request.query(`EXEC FindInboundOrdersByTimePeriod null, null`);
        res.render('nhaphang', { dulieu: result.recordset });
    }
});
